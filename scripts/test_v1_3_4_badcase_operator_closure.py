...
def run(base_url: str) -> int:
    ...
    print("\n[5b] Skill / capability gap reality checks (no LLM required)")
    # Create a skill_prompt case and manually seed a skill draft via Darwin if possible.
    create_resp = c.post("/api/badcases", {
        "title": "验收测试：Skill 草稿拦截",
        "description": "用于验证 Skill 草稿应用被拦截",
        "category": "skill_prompt",
        "source": "manual",
        "original_query": "skill 测试",
        "ai_response": "skill 回答",
    })
    skill_case_id = create_resp["badcase"]["id"]
    ...
    create_resp = c.post("/api/badcases", {
        "title": "验收测试：能力缺口保持 fixing",
        "description": "用于验证能力缺口应用后仍保持 fixing",
        "category": "mcp_capability",
        "source": "manual",
        "original_query": "mcp 测试",
        "ai_response": "mcp 回答",
    })
    gap_case_id = create_resp["badcase"]["id"]
    ...
    print("\n[5c] Draft review cannot bypass apply")
    # Reuse the knowledge case; create another draft and try to review it directly to published.
    c.post(f"/api/badcases/{case_id}/transition", {"status": "fixing"})
    c.post(f"/api/badcases/{case_id}/extract-knowledge", {
        "title": "绕过测试草稿",
    ...
    detail = c.get(f"/api/badcases/{case_id}")["badcase"]
    bypass = find_draft(detail["knowledge_drafts"], "绕过测试草稿")
    if bypass:
        check(
            "review cannot set knowledge draft to published",
            lambda: c.expect_error(
                f"/api/badcases/{case_id}/knowledge-drafts/{bypass['id']}/review",
