...
def _structured_payload(value: Any) -> Dict[str, Any]:
    """Project a validated model object without interpreting natural language."""
    ...
    if not isinstance(value, dict):
        raise TypeError("structured work-order request must be an object")
    return dict(value)


def _validated_work_order_parameters(value: Any) -> Dict[str, str]:
    """Validate Agent-supplied fields; never derive them from message text."""

    raw = _structured_payload(value)
    unknown = sorted(set(raw) - set(WORK_ORDER_FIELDS))
    if unknown:
        raise ValueError("work order payload has unknown fields: " + ", ".join(unknown))
    normalized: Dict[str, str] = {}
    for field in WORK_ORDER_FIELDS:
        text = str(raw.get(field) or "").strip()
        if len(text) > 500:
            raise ValueError(f"work order field is too long: {field}")
        normalized[field] = text
    return normalized


def advance_structured_work_order_workflow(
    *,
    session_id: str,
    selected_agent_id: str,
    selected_agent_scope: str,
    trace_id: Optional[str] = None,
    release_id: Optional[str] = None,
    proposal_request: Optional[Any] = None,
    confirmation_request: Optional[Any] = None,
) -> Optional[Dict[str, Any]]:
    """Advance the write state machine only from one selected B Agent envelope.

    This is the production entry point.  It accepts no user message and performs
    no keyword, regex or semantic interpretation.  The selected Agent supplies
    one strict structured request after seeing the complete conversation.  A
    proposal request may create/update a Draft and, once complete, a pending
    Proposal.  Only a later structured confirmation request may approve and
    execute the internal ``work_order.create`` service.
    """

    agent_id = str(selected_agent_id or "").strip()
    if not agent_id or selected_agent_scope != "property":
        raise PermissionError("only the selected B property Agent may request a work order")
    if proposal_request is not None and confirmation_request is not None:
        raise ValueError("Agent envelope must contain at most one work-order request")
    if proposal_request is None and confirmation_request is None:
        return None

    if proposal_request is not None:
        request = _structured_payload(proposal_request)
        expected_fields = {"action_type", *WORK_ORDER_FIELDS}
        if set(request) != expected_fields:
            raise ValueError(
                "proposal_request must contain action_type and the exact work-order fields"
            )
        if request.get("action_type") != ACTION_TYPE:
            raise ValueError("unsupported structured action type")

        incoming = _validated_work_order_parameters(
            {field: request.get(field) for field in WORK_ORDER_FIELDS}
        )
        existing = get_work_order_draft(session_id) or {}
        draft = {
            field: incoming.get(field) or str(existing.get(field) or "").strip()
            for field in WORK_ORDER_FIELDS
        }
        save_work_order_draft(session_id=session_id, **draft)
        missing = [field for field in WORK_ORDER_REQUIRED_FIELDS if not draft[field]]
        if missing:
            return _result(
                "draft_updated",
                "维修工单草稿已保存，尚未形成待确认 Proposal。仍缺少结构化字段："
                + "、".join(missing)
                + "。",
                draft,
        ...
        if pending and (pending.get("payload") or {}) != draft:
            raise RuntimeError(
                "a different work-order Proposal is already pending confirmation"
            )
        ...
        if proposal.status != "pending_confirmation":
            raise RuntimeError(
                "idempotent work-order Proposal is no longer pending confirmation"
            )
        return _result(
            "awaiting_confirmation",
            "维修工单 Proposal 已创建并停在待确认状态；尚无 Approval、Receipt 或正式工单。",
            draft,
    ...
    request = _structured_payload(confirmation_request)
    if set(request) != {"action_type", "proposal_id", "decision"}:
        raise ValueError(
            "confirmation_request must contain action_type, proposal_id and decision only"
        )
    if request.get("action_type") != ACTION_TYPE:
        raise ValueError("unsupported structured action type")
    ...
    if not proposal_id or decision not in {"approve", "reject"}:
        raise ValueError("invalid structured confirmation request")
    proposal_row = get_action_proposal(proposal_id)
    if not proposal_row:
        raise ValueError("work-order Proposal not found")
    if proposal_row.get("session_id") != session_id:
        raise PermissionError("work-order Proposal belongs to another Session")
    if proposal_row.get("action_type") != ACTION_TYPE:
        raise PermissionError("confirmation target is not a work-order Proposal")

    status = str(proposal_row.get("status") or "")
    if decision == "reject":
        if status != "pending_confirmation":
            raise PermissionError("only a pending Proposal may be rejected")
        rejected = action_gateway.reject(
            proposal_id,
            actor=f"owner:{session_id}",
            comment=f"selected B Agent {agent_id} returned structured rejection",
        )
        delete_work_order_draft(session_id)
        return _result(
            "rejected",
            "待确认的维修工单 Proposal 已拒绝；未产生 Receipt 或正式工单。",
            {},
    ...
    if status == "committed":
        receipt = get_action_receipt_by_idempotency_key(
            str(proposal_row.get("idempotency_key") or "")
        ) or {}
        return _result(
            "idempotent_replay",
            "该 Proposal 已提交；本次结构化确认未重复写入。",
            {},
            proposal_id=proposal_id,
            receipt=receipt,
            work_order_id=receipt.get("resource_id"),
            selected_agent_id=agent_id,
        )
    if status != "pending_confirmation":
        raise PermissionError("only a pending Proposal may be approved")

    approved = action_gateway.approve(
        proposal_id,
        actor=f"owner:{session_id}",
        comment=f"selected B Agent {agent_id} returned structured owner confirmation",
    )
    if approved.status != "approved":
        raise RuntimeError("work-order Proposal approval did not commit")
    receipt = action_gateway.execute(proposal_id)
    if not receipt.may_claim_success:
        return _result(
            "failed",
            "内部服务未返回 committed Receipt；不得声称工单创建成功。",
            get_work_order_draft(session_id) or {},
            proposal_id=proposal_id,
    ...
    return _result(
        "committed",
        f"维修工单已由内部服务创建，工单号：{receipt.resource_id}。",
        {},
        proposal_id=proposal_id,
        receipt=receipt.model_dump(mode="json"),
